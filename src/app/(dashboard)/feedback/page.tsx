...
export default function FeedbackPage() {
  const [orderId,       setOrderId]       = useState(MOCK_ORDERS[0].id);
  ...
  const selectedOrder  = MOCK_ORDERS.find((o) => o.id === orderId) ?? MOCK_ORDERS[0];
  const selectedCredit = MOCK_CREDITS.find((c) => c.credit_code === selectedOrder.credit_code);
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            >
              {MOCK_ORDERS.map((o) => {
                const c = MOCK_CREDITS.find((cr) => cr.credit_code === o.credit_code);
                return (
                  <option key={o.id} value={o.id}>
                    #{o.id.slice(-6).toUpperCase()}: {c ? `${c.credit_code} ${c.credit_name}` : "Order"}
                  </option>
                );
              })}
            </select>
            {selectedCredit && (
              <p className="text-xs text-certify-cool-grey mt-1.5 ml-1">{selectedCredit.credit_code} · {selectedCredit.category}</p>
            )}
          </div>
