...
Walking distance maps, site context maps, transit maps, bicycle maps \u2014 all maps are produced by the Google Maps API pipeline, not by you. Two placeholders are used \u2014 use the correct one for the context:

1. In the LEED Online Submittal Form section, where the form has a map upload field: <!-- WALKING_DISTANCE_MAP -->
2. In Supporting Documentation, where the actual map image should appear: <img data-map-insert="1" alt="Walking distance map">

Never produce an SVG map. Never produce a drawn map. Never produce a map of any kind. Both placeholders are replaced programmatically after your response is complete.
...
${plainText}`
      ...
      if (jsonMatch) {
        ...
        const extracted = raw.filter((l) => l && typeof l.address === "string" && l.address.trim().length > 5).map((l) => ({ address: l.address.trim(), label: String(l.label ?? "") }));
      ...
      console.warn(`  Step 15.7b: Haiku extraction failed: ${err.message} \u2014 map skipped`);
    }
  }
  if (requiredMapType && project.address && locationsForMap.length > 0) {
    ...
    try {
      ...
      console.warn(`  Step 15.8b: Map generation failed: ${e.message} \u2014 continuing without map`);
    }
